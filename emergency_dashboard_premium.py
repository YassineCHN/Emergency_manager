...
from premium_styles_v3_FIXED import get_premium_css_v3
from dashboard_components_v2 import (
    render_hero_zone,
...
if st.session_state.current_tab == "dashboard":
    ...
    #     if st.session_state.show_chat_panel and CHATBOT_AVAILABLE and st.session_state.get('chatbot'):
    #         # Layout 2 colonnes : 65% contenu + 35% panneau
    #         col_main, col_panel = st.columns([65, 35], gap="small")
    #     else:
    #         # Pas de panneau : container normal
    #         col_main = st.container()
    #         col_panel = None
    ...
    # if st.session_state.show_chat_panel and CHATBOT_AVAILABLE and st.session_state.get('chatbot'):
    #     # Réduire le contenu principal à 60%
    #     # Note: En pratique, Streamlit ne permet pas de layout dynamique complexe
    #     # Le panneau sera affiché en overlay ou dans une colonne
...
#     if not MONITORING_AVAILABLE:
#         st.error("❌ Module monitoring non disponible. Installez `monitoring/monitoring.py`")
#     else:
#         # Métriques globales
#         st.markdown("### 📋 Métriques Globales")
...
#                 help="Consommation énergétique estimée"
#             )
#         with col3:
#             st.metric(
#                 label="🌍 Empreinte CO2",
#                 value=f"{monitor.total_co2_kg:.6f} kg",
#                 help="kgCO2eq - Mix France"
#             )
#         with col4:
#             avg_latency = monitor.get_average_latency()
#             st.metric(
...
# ========== CYCLE AGENT (SI DASHBOARD ACTIF) ==========
...
# # Imports composants V2 (réutilisés)
# from premium_styles_v3 import get_premium_css_v3
# from dashboard_components_v2 import (
#     render_hero_zone,
#     render_critical_situation_zone,
#     render_kpi_secondary,
...
# except ImportError:
#     MONITORING_AVAILABLE = False
#     print("⚠️ Module monitoring non disponible")
...
#         "Lucas", "Hugo", "Léa", "Chloé", "Nathan", "Camille", "Antoine",
#         "Nicolas", "Sarah", "Alexandre", "Charlotte", "Maxime", "Laura", "Julien",
#         "Océane", "Mathieu", "Pauline", "Raphaël", "Manon", "Benjamin", "Clara",
#         "Romain", "Louise", "Théo", "Zoé", "Louis", "Alice", "Gabriel", "Inès",
#         "Arthur", "Jade", "Tom", "Lola", "Paul", "Lily", "Enzo", "Anna",
#         "Adam", "Rose", "Victor", "Eva", "Jules", "Mia", "Ethan", "Nina",
#         "Mathis", "Lucie", "Noah", "Amélie", "Clément", "Anaïs", "Simon",
#         "Margaux", "Baptiste", "Justine", "Valentin", "Emilie", "Adrien",
#         "Melissa", "Bastien", "Aurore", "Damien", "Fanny", "Kevin", "Coralie",
#         "Anthony", "Elise", "David", "Céline", "Florian", "Audrey", "Quentin"
#     ]
...
#     symptomes_map = {
#         Gravite.ROUGE: ["Douleur thoracique intense", "Difficulté respiratoire sévère",
#                         "Perte de conscience", "Hémorragie importante"],
#         Gravite.JAUNE: ["Fracture suspectée", "Douleurs abdominales",
#                         "Fièvre élevée persistante", "Vertiges importants"],
#         Gravite.VERT: ["Entorse cheville", "Plaie superficielle", "Fièvre modérée", "Mal de dos"],
#         Gravite.GRIS: ["Consultation routine", "Renouvellement ordonnance",
#                        "Certificat médical", "Contrôle de suivi"]
#     }
...
#         if assign_result["success"]:
#             salle_id = assign_result["salle_id"]
#             add_event(f"Patient {patient.prenom} {patient.nom} assigné à {salle_id}", "🏥")
#         else:
#             add_event(f"⚠️ {patient.prenom} {patient.nom} - Échec assignation", "❌")
#     else:
#         add_event(f"❌ {patient.prenom} {patient.nom} - Échec création", "❌")
...
#         staff_mobiles = [s for s in self.state.staff if s.type.value in ["infirmier(ere)_mobile", "aide_soignant"]]
#         staff_dispo = [s for s in staff_mobiles if s.disponible and not s.en_transport]
#         if len(staff_dispo) < 2: return None
#         queue = self.state.get_queue_consultation()
#         if queue and staff_dispo:
...
#     def _gerer_sortie_consultation(self) -> Optional[str]:
#         if self.state.consultation.est_libre(): return None
#         pid = self.state.consultation.patient_id
#         patient = self.state.patients.get(pid)
#         debut = self.state.consultation.debut_consultation
...
#         # Agent
#         st.markdown("### 🤖 Agent")
#         st.session_state.agent_enabled = st.checkbox("Activer l'agent", value=st.session_state.agent_enabled)
#         if st.session_state.agent_enabled:
#             st.success("✅ Agent actif")
#         else:
#             st.warning("⏸️ Agent désactivé")
#         st.markdown("**Vitesse agent**")
#         st.session_state.agent_speed = st.slider("Vitesse (s)", 0.1, 2.0, st.session_state.agent_speed, 0.1, label_visibility="collapsed")
...
#         st.markdown("### 📊 Stats Agent")
#         st.markdown(f"**Actions prises**")
#         st.markdown(f"<h2 style='color: #00D084; font-size: 2rem; margin: 0;'>{st.session_state.actions_count}</h2>", unsafe_allow_html=True)
...
#     # Calculs KPI
#     nb_total = len([p for p in patients.values() if p.get("statut") != "sorti"])
#     nb_rouge_attente = len([p for p in patients.values()
#                             if p.get("gravite") == "ROUGE" and p.get("statut") == "salle_attente"])
...
#         col1, col2, col3 = st.columns(3)
#         with col1:
#             render_staff_section_with_tension("Médecins", "👨‍⚕️", medecins, 1,
#                                             is_medecin=True, consultation_occupee=consultation_occupee)
#         with col2:
#             render_staff_section_with_tension("Inf. Mobiles", "🏃", inf_mobiles, 2)
#         with col3:
...
# if st.session_state.running and st.session_state.agent_enabled and st.session_state.current_tab == "dashboard":
#     if st.session_state.agent is None:
